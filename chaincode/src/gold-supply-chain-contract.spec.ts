...
// import { LocationData } from './models/product-location-data';
...
//     locationData.current = new LocationEntry({
...
//             await contract.goldExists(ctx, '1001').should.eventually.be.true;
//         });

//         it('should return false for a product that does not exist', async () => {
//             await contract.goldExists(ctx, '1003').should.eventually.be.false;
//         });
//     });
...
//             // act
//             await contract.shipGoldTo(ctx, productId, newLocation, newLocationArrivalDate);

//             // assert
//             ctx.stub.putState.should.have.been.calledWith(productId, sinon.match((data: Buffer) => {
//                 const updatedProduct = JSON.parse(data.toString()) as Gold;
...
//             // act
//             await contract.shipGoldTo(ctx, productId, 'New Location', '2021-07-01T18:00:58.511Z');
...
//             await contract.shipGoldTo(ctx, '1003', 'New Location', '2021-06-24T18:25:43.511Z').should.be.rejectedWith(/The product 1003 does not exist./);
//         });

//         it('should throw an error if new location is empty', async () => {
//             await contract.shipGoldTo(ctx, '1001', '', '2021-06-24T18:25:43.511Z').should.be.rejectedWith(/The 'newLocation' field is required./);
//         });

//         it('should throw an error if new location arrival date is empty', async () => {
//             await contract.shipGoldTo(ctx, '1001', 'New Location', '').should.be.rejectedWith(/The 'arrivalDate' field is required./);
//         });
//     });

//     describe('#getProduct', () => {
//         it('should return a product', async () => {
//             const expectedProduct = new Gold();
...
//             const childProduct1 = new GoldWithHistory();
//             childProduct1.id = childProduct1Id;
//             const childProduct2 = new GoldWithHistory();
//             childProduct2.id = childProduct2Id;

//             rootProduct.componentIds = [childProduct1Id, childProduct2Id];
//             rootProduct.component = [childProduct1, childProduct2];
...
//             await contract.getProductWithHistory(ctx, '1001').should.eventually.satisfy((p: GoldWithHistory) => {
//                 return p.id === rootProductId &&
//                     p.component[0] &&
//                     p.component[0].id === childProduct1Id &&
//                     p.component[1] &&
//                     p.component[1].id === childProduct2Id;
//             });
//         });
